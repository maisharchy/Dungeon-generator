...
void io_init_terminal(void)
{
  ...
  short pairs[] = { COLOR_RED, COLOR_GREEN, COLOR_YELLOW, COLOR_BLUE,
                    COLOR_MAGENTA, COLOR_CYAN, COLOR_WHITE };

  for (short i = 0; i < 7; ++i) {
    init_pair(pairs[i], pairs[i], COLOR_BLACK);
    attron(COLOR_PAIR(pairs[i]));
  }
...
{
    uint32_t y, x;
    uint32_t is_illuminated_tile;
    character *visible_character;
    int32_t visible_count = 0;

    clear();
    std::cout << "Total Objects: " << d->num_object << std::endl;
    for (y = 0; y < 21; y++)
    {
        for (x = 0; x < 80; x++)
        {
            is_illuminated_tile = is_illuminated(d->PC, y, x);
            if (is_illuminated_tile)
            {
                attron(A_BOLD);
            }/*
            if (d->object_map[y][x]) {
              attron(COLOR_PAIR(d->object_map[y][x]->color));
              mvaddch(y, x, d->object_map[y][x]->symbol);
              attroff(COLOR_PAIR(d->object_map[y][x]->color));
            }*/
            if (d->character_map[y][x] && can_see(d,
    ...
    mvprintw(23, 1, "Player Position: (%2d,%2d).", d->PC->position[dim_x], d->PC->position[dim_y]);
    mvprintw(22, 1, "%d visible %s.", visible_count, visible_count > 1 ? "monsters" : "monster");
    mvprintw(22, 30, "Closest visible monster: ");
    
    visible_character = io_nearest_visible_monster(d);
    if (visible_character)
    {
        attron(COLOR_PAIR(COLOR_RED));
        mvprintw(22, 55, "%c at %d %c and %d %c.",
                 visible_character->symbol,
                 abs(visible_character->position[dim_y] - d->PC->position[dim_y]),
                 (visible_character->position[dim_y] - d->PC->position[dim_y] <= 0 ? 'N' : 'S'),
                 abs(visible_character->position[dim_x] - d->PC->position[dim_x]),
                 (visible_character->position[dim_x] - d->PC->position[dim_x] <= 0 ? 'W' : 'E'));
...
void io_display_no_fog(dungeon *d)
{
    ...
    mvprintw(23, 1, "Player Position: (%2d,%2d).", d->PC->position[dim_x], d->PC->position[dim_y]);
    mvprintw(22, 1, "%d %s.", d->num_monsters, d->num_monsters > 1 ? "monsters" : "monster");
    mvprintw(22, 30, "Closest visible monster: ");
...
static void io_list_monsters_display(dungeon *d, character **c, uint32_t count)
{
  uint32_t i;
  char (*s)[40]; 
  ...
  for (i = 0; i < count; i++)
  {
    snprintf(s[i], 40, "%s%c: %d %s, %d %s",
             (c[i]->symbol == 'd' ? "Tenacious " : adjectives[rand() % (sizeof(adjectives) / sizeof(adjectives[0]))]),
             c[i]->symbol,
             abs(c[i]->position[dim_y] - d->PC->position[dim_y]),
             ((c[i]->position[dim_y] - d->PC->position[dim_y]) <= 0 ? "N" : "S"),
             abs(c[i]->position[dim_x] - d->PC->position[dim_x]),
             ((c[i]->position[dim_x] - d->PC->position[dim_x]) <= 0 ? "W" : "E"));
