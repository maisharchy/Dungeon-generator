...
{
  std::vector<monster_description> &descs = d->monster_descriptions;
  d->num_monsters = descs.size();

  for (uint32_t idx = 0; idx < descs.size(); ++idx) {
    npc *new_mon = descs[idx].gen_monster();
    bool placed = false;
    while (!placed) {
      uint32_t chosen_room = rand_range(1, d->num_rooms - 1);
      int y = rand_range(d->rooms[chosen_room].position[dim_y],
                        d->rooms[chosen_room].position[dim_y] + d->rooms[chosen_room].size[dim_y] - 1);
      int x = rand_range(d->rooms[chosen_room].position[dim_x],
                        d->rooms[chosen_room].position[dim_x] + d->rooms[chosen_room].size[dim_x] - 1);
      if (!d->character_map[y][x]) {
        new_mon->position[dim_y] = y;
        new_mon->position[dim_x] = x;
        d->character_map[y][x] = new_mon;
        heap_insert(&d->events, new_event(d, event_character_turn, new_mon, 0));
        placed = true;
      }
    }
  }
...
void (*npc_move_func[])(dungeon *d, npc *c, pair_t next) = {
  npc_next_pos_00, npc_next_pos_01, npc_next_pos_02, npc_next_pos_03,
  npc_next_pos_04, npc_next_pos_05, npc_next_pos_06, npc_next_pos_07,
  npc_next_pos_08, npc_next_pos_09, npc_next_pos_0a, npc_next_pos_0b,
  npc_next_pos_0c, npc_next_pos_0d, npc_next_pos_0e, npc_next_pos_0f
};
