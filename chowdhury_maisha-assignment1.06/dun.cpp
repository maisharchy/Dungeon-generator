...
void init_seen();
void render_dungeon(Position player_pos);
...
#define VISIBLE 'V'
#define UNSEEN ' ' // 'rock' to represent unseen areas

char seen[HEIGHT][WIDTH]; // 2D array to track visible areas
bool fog_of_war = true; // Track if fog of war is on

void init_seen() {
    for (int y = 0; y < HEIGHT; y++) {
        for (int x = 0; x < WIDTH; x++) {
            seen[y][x] = UNSEEN; // Initially all areas are unseen
        }
    }
}
...
int max_visible_size = 5;

#define MAX_VISIBILITY 80  // or dungeon width

void update_seen(Position player_pos) {
    int radius = max_visible_size / 2;
    
    for (int dy = -radius; dy <= radius; dy++) {
        for (int dx = -radius; dx <= radius; dx++) {
            int nx = player_pos.x + dx;
            int ny = player_pos.y + dy;

            if (nx >= 0 && nx < WIDTH && ny >= 0 && ny < HEIGHT) {
                seen[ny][nx] = VISIBLE; // Mark tile as visible
            }
        }
    }
...
void render_dungeon(Position player_pos) {
    for (int y = 0; y < HEIGHT; y++) {
        for (int x = 0; x < WIDTH; x++) {
            if (seen[y][x] == VISIBLE) {
                printf("%c", dungeon[y][x]);  // Show revealed areas
            } else if (seen[y][x]) {
                printf(" ");  // Previously seen but currently hidden
            } else {
                printf(" ");  // Unexplored
            }
        }
...
void update_visible_area() {
    if (max_visible_size < MAX_VISIBILITY) {
        max_visible_size += 5; 
    }
}
void teleport_mode(Position *player_pos) {
    Position cursor = *player_pos;
    char input;
    bool teleporting = true;

    while (teleporting) {
        draw_dungeon();
        mvaddch(cursor.y, cursor.x, '*');  // Show cursor
        refresh();

        input = getchar();
        switch (input) {
            case 'h': cursor.x = std::max(0, cursor.x - 1); break;  // Use std::max
            case 'l': cursor.x = std::min(WIDTH - 1, cursor.x + 1); break;  // Use std::min
            case 'k': cursor.y = std::max(0, cursor.y - 1); break;  // Use std::max
            case 'j': cursor.y = std::min(HEIGHT - 1, cursor.y + 1); break;  // Use std::min
            case 'g': // Confirm teleport
    if (dungeon[cursor.y][cursor.x] != ROCK && seen[cursor.y][cursor.x] == VISIBLE) {
...
case 'r': {
    ...
    do {
        cursor.x = rand() % WIDTH;
        cursor.y = rand() % HEIGHT;
        attempts--;
    } while ((dungeon[cursor.y][cursor.x] == ROCK || seen[cursor.y][cursor.x] != VISIBLE) && attempts > 0);
...
void handle_input(Position *player_pos) {
    ...
    printf("  q to quit\n");
    printf("  f to toggle fog of war\n");
    printf("  g to enter teleport mode\n");
    printf("  s to get terrain map full\n");
    ...
    switch(input) {
        ...
        case 'h':
            ...
            break;
        case 'f': 
            fog_of_war = !fog_of_war;  // Toggle fog of war
            break;
        case 'g':
            teleport_mode(player_pos);
            render_dungeon(*player_pos);
            break;  
        case 's':
            update_display(*player_pos);
...
int main(int argc, char *argv[]) {
    srand(time(NULL));  
    std::string monster_file_path = std::string(getenv("HOME")) + "/.rlg327/monster_desc.txt"; 
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--monsterfile") == 0 && i + 1 < argc) {
            monster_file_path = argv[i + 1];
            i++; 
        }
    }
    std::vector<Monsterr> monsterr;
    if (parse_monsters(monster_file_path, monsterr)) {
        std::cout << monsterr.size() << " monsters parsed from"<< monster_file_path <<".\n";//debugging
    } else {
        std::cerr << "Failed to load monsters from " << monster_file_path << ".\n";
    }
    std::vector<Item> items;
    std::string item_file_path = std::string(getenv("HOME")) + "/.rlg327/object_desc.txt"; 
    ItemParser::parse_items(item_file_path, items);
    std::cout <<items.size() << " items parsed from"<< item_file_path <<".\n\n";//debugging
    for (const auto& item : items) {
        ItemParser::print_item(item);
    }
//game starter commented out for parsing assignment    
/*
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--save") == 0) {
            if (save_monsters(monster_file_path, monsterr)) {
                std::cout << "Successfully saved monsters to " << monster_file_path << ".\n";
            } else {
                std::cerr << "Failed to save monsters to " << monster_file_path << ".\n";
            }
            break;  
        }
    }
